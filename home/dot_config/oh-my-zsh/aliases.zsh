...
# To match uncompress, unxz, unzip, unzstd
alias \
  unbzip2="bunzip2" \
  ungz="gunzip" \
  untar="tar --extract --file"
...
# in UTC timezone since `date -Iseconds` provides timezone offset
alias \
  now="date -u +%FT%TZ" \
  nowtz="date -Iseconds" \
  iso8601="now" \
  rfc2822="date -R" \
  timestamp="date +%s" \
  timestampms="expr $(date +%s%N) / 1000000" \
